...
export class ObjectiveContainerComponent {
  ...
  handleCellClick(objectiveCell : ObjectiveCell) {
    if (!objectiveCell.isChecked || this.findObjectiveItemForCell(objectiveCell)?.isReversible) {
  ...
  findObjectiveItemForCell(objectiveCell : ObjectiveCell) : ObjectiveItem {
    return this.objectiveItemList.filter(item => item.objectiveCellList.filter(cell => cell == objectiveCell).length > 0)[0];
  }
}
